...
def runtime_block(world: World) -> str:
    ...
    return "\n".join(
        [
            ...
            "find: <find limit=\"20\">fragment</find> — frecency-ranked fuzzy path search over cwd (fff), one line per hit as path<TAB>score. Your edits feed the ranking. Path search only; bash/rg owns content grep. Absent build refuses and names scripts/build-fff-python.sh.",
