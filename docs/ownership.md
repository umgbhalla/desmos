...
cross-process ranking channel: the kernel writes it (`workspace op=edit` → `touch`), and a
reader — the `workspace op=find` engine today, the TUI fuzzy picker once wired — reads it
one-way. It never flows the other direction.

| surface | writer | reader |
|---|---|---|
| `<cwd>/.desmos/fff` — the fff frecency LMDB (path-search ranking). Recently-`workspace op=edit`-touched paths rank higher without the model asking. | `state/find.py touch()` at the `workspace op=edit` dispatch choke point (`kernel/dispatch.py`, `line is not None` branch) → `track_access`; a live `FileFinder` writes it under `workspace op=find` | `state/find.py find()` (`workspace op=find` ranking); **one-way** to the TUI — `crates/desmos-tui/src/fuzzy.rs` reads scores cached at scan time (frame wiring pending), never writes |
