...
- 6.5 memex fork (`memex-desmos`, pinned rev): memex has no adapter
  ...
  the fork lives at github.com/umgbhalla/memex-desmos, pinned as the
  `vendor/memex` submodule; scripts/memex-setup.sh inits it and builds an
  external binary (tantivy+usearch+ort must not enter our build). Probe =
  `memex search __probe__ --source desmos --limit 1` exit code (stock memex
  rejects the label; the query token is required or the fork also exits
  nonzero) — silent when absent, loud when present and wrong. Never parse
  trajectory/*.json or harness.sqlite3.
