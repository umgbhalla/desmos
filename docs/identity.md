...
|---|---|---|---|---|---|---|---|
| code + prompts (ABI `kernel/const.py`, catalog `kernel/catalog.py`, docs/, packaged skills `desmos/skills/`) | git worktree | humans + the model via `workspace op=edit` (`kernel/edit.py`) | yes — reload re-reads it | yes | yes | yes | **yes** |
...
| fff frecency LMDB (path-search ranking; `workspace op=find` reads it, recently-`workspace op=edit`-touched paths rank higher) | `<cwd>/.desmos/fff` | `state/find.py touch()` at the `workspace op=edit` dispatch choke point → `track_access` (every world, root and child, by construction); a live `FileFinder` writes it under `workspace op=find` | yes — file on disk; the module-global `_ENGINES` survives `reload_sdk` via `globals().get` | yes — file persists; the in-memory engine dies and rebuilds lazily on the next `workspace op=find` | no — snapshots carry notes/tools/prior only | no | no |
...
| skills | packaged `desmos/skills/`; shared `~/.agents/skills`; user `~/.desmos/skills`; project chain `<dir>/.agents/skills` + `<dir>/.desmos/skills` up to the git root (`skills/__init__.py skill_roots`, rediscovered every turn) | the model via `workspace op=edit`/bash | yes | yes | yes | only the project `.desmos/skills` root dies | packaged root only |
| extensions | `~/.desmos/extensions` + project chain `.desmos/extensions` (`state/extensions.py extension_roots`, loaded per turn) | the model via `workspace op=edit`/bash | yes | yes | yes | project root dies; user root survives | no |
...
|---|---|---|
| TUI fuzzy file picker read of the fff LMDB (one-way, kernel writes → TUI reads at scan time) | `crates/desmos-tui/src/fuzzy.rs` | the module exists but the frame (ctrl-key open, overlay draw, worker `poll`) is not wired in `main.rs`/`app.rs`/`input.rs` yet (6.4). The LMDB writer and the `workspace op=find` reader are already live above. |
