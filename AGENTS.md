...
| Same text on the story pane and the calls pane | Story is grok `UserPrompt` / `AgentMessage`. Thinking is Activity on the TUI; ACP tags it Story for Comet Thought. Calls are grok `ToolCall` for `complete()` and syscalls. Do not mirror. Do not stamp everything `out`. |
...
- Default TUI is `python -m desmos tui` → `crates/desmos-tui` hosting grok-build `ScrollbackState` / `ScrollbackPane`. Story = UserPrompt / AgentMessage / work-run (edits excluded) / subagent. Thinking is Activity `Thinking`. ACP/Comet/Desk tag thinking as Story (`agent_thought_chunk` / Comet `MessagePart::Thought`) — two paints of the same kernel event, not a second source. Calls = ToolCall. Tab/j/k/h/l, click select, double-click fold. Needs `vendor/grok-build`. Do not flatten blocks to an `out` label.
...
- `vendor/grok-build` is a pinned git submodule of the `umgbhalla/grok-build` fork, not flat source in this repository. A fresh checkout must run `git submodule update --init vendor/grok-build` before a source build. Hyperion, IX, and release jobs must initialize it themselves. Do not vendor a half-copy of one crate and call that "using grok-build," and do not re-gitignore the gitlink: a `path = ` dep that exists only on your disk still breaks for the next clone.
- A change to a grok-build crate is a commit in the submodule fork. Push that commit, then stage the updated `vendor/grok-build` gitlink in the root commit that needs it. There is no patch series: an unpushed submodule commit or an unstaged gitlink exists only on your machine while the local build stays green.
- `python -m desmos check` guards exactly two things here. `_check_path_deps_tracked` asks git — not the filesystem — whether every `path = ` dep in the root `Cargo.toml` is tracked directly or through a committed gitlink; it exists because a bare `build/` in a global gitignore once swallowed `crates/build/xai-proto-build`. `_check_vendor_patch` asserts the pinned pager still carries our DESMOS_ACP branch (`std::env::var("DESMOS_ACP")` in `acp/mod.rs`, `pub async fn spawn_stdio_acp` in `acp/spawn.rs`), because moving the gitlink to an incompatible fork commit can compile fine and silently hand `--grok` back to grok's own in-process agent.
- The TUI build is hash-gated and PROTOC-pinned: `python -m desmos tui` hashes our sources (`desmos/front/hashgate.py`, shared with Comet and `desmos-md-html`) and only shells out to cargo when they moved, and `.cargo/config.toml` forces `PROTOC=scripts/protoc` so the pager graph does not rebuild every launch. Do not bypass the wrapper or bolt RUSTFLAGS onto that path.
...
- `<edit>` stays wire-only. Do not restore story edit cards — that prints `edit ×3` above three cards naming the files. Edits are therefore excluded from the work-run sentence. Every other tag stays wire-only too.
