...
impl FileFinder {
    ...
    /// Record an access of `path` in the frecency DB and refresh its in-memory
    /// score. Port of the fff-nvim frecency pair, split so the LMDB write does
    /// not need the file index: the frecency write runs unconditionally (as
    /// long as a frecency tracker is open), and the in-memory item update only
    /// happens when a picker index already exists. This lets a consumer feed
    /// frecency (e.g. on every edit) without paying for a full scan first.
    /// Returns true when the frecency write happened, false when no tracker is
    /// open.
    fn track_access(&self, py: Python<'_>, path: PathBuf) -> PyResult<bool> {
        let frecency = self.frecency.clone();
        let picker = self.picker.clone();
        py.allow_threads(move || -> PyResult<bool> {
            // Unconditional frecency LMDB write (needs no index, no picker).
            {
                let guard = frecency.read().map_err(py_err)?;
                let Some(tracker) = guard.as_ref() else {
                    return Ok(false);
                };
                tracker.track_access(&path).map_err(py_err)?;
            }
            // In-memory item update only when an index exists; a miss is fine.
            if let Ok(mut guard) = picker.write() {
                if let Some(pick) = guard.as_mut() {
                    let fre_guard = frecency.read().map_err(py_err)?;
                    if let Some(tracker) = fre_guard.as_ref() {
                        pick.update_single_file_frecency(&path, tracker)
                            .map_err(py_err)?;
                    }
                }
            }
